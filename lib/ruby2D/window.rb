class Ruby2D::Window
  ...
  @procs[:keyboard_up] = lambda do |key,x,y|
    Ruby2D::Input.feed key.chr => :release
    gm = GLUT.GetModifiers
    case gm
    when 1 ; Input.feed 'maj' => :press
    when 2 ; Input.feed 'ctrl' => :press
  ...
  @procs[:special_keyboard] = lambda do |key,x,y|
    Ruby2D::Input.feed get_special_key(key) => :trig
  end
  @procs[:special_keyboard_up] = lambda do |key,x,y|
    Ruby2D::Input.feed get_special_key(key) => :release
  end
  @procs[:mouse] = lambda do |key,state,x,y|
    ...
    when 1 ; nkey = :middle
    when 2 ; nkey = :right
    when 3 ; nkey = :wheel_Up
    when 4 ; nkey = :wheel_Down
    end
    case state
    when 0 ; nstate = :down
    when 1 ; nstate = :up
    end
    Ruby2D::Mouse.feed nkey => nstate, :x => x, :y => y
  end
  @procs[:mouse_passive] = lambda do |x,y|
    Ruby2D::Mouse.feed :x => x, :y => y
  end
...
class << Ruby2D::Window
  ...
  def get_special_key n
    %w(F1 F2 F3 F4 F5 F6 F7 F8 F9 F10 F11 F12
    DOWN LEFT RIGHT UP PAGE_UP PAGE_DOWN
    END HOME INSERT).each {|k| return k if GLUT.const_get('KEY_'+k) == n}
  end
end